...
|---|---:|---|---|
| SQL-자격검정-실전문제.pdf | 144 | 전체 텍스트 추출, 대표 문제/해설/실기 페이지 렌더링 확인, 상세 분석 문서 작성 | SQLP 필기 객관식 출제 스타일, 선택지별 해설 품질, SQL 전문가 실기 문제 구성 |
...
- `SQL-자격검정-실전문제.pdf` 대표 페이지: 데이터 모델링 문제 페이지, 해설 페이지, Lock/동시성 해설, SQL 전문가 실기 정답 페이지를 이미지로 렌더링해 확인했다.
...
- `SQL-자격검정-실전문제.pdf`에서 확인한 문제 구성 방식에 따라 표, ERD, SQL, 실행 결과, 실행계획, Trace 자료를 필요할 때 함께 제공한다.
...
# 2026-07-23 Official PDF Extraction Update

The uploaded PDFs are now treated as SQLMate's official source corpus. Extraction is performed by `scripts/extract-official-pdf-bank.py` and saved to `data/official-pdf-question-units.json`.

Extraction version: `official-pdf-extracted-2026-07-23`

| Source | Pages |
|---|---:|
| SQL-자격검정-실전문제.pdf | 144 |
| 45회_기출문제.pdf | 20 |
| 46회_기출문제.pdf | 11 |
| 47회_기출문제.pdf | 12 |
| 48회_기출문제.pdf | 14 |
...
The active bank now uses extracted units as the source of truth and creates Original, Safe Variant, and Similar entries. See `docs/OFFICIAL_PDF_CORPUS_ANALYSIS.md`, `docs/QUESTION_COVERAGE_REPORT.md`, and `docs/CONTENT_QA_REPORT.md` for the current counts and limitations.
